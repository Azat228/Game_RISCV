...
void save_all_scores() {
    uint16_t addr = SCORE_START_ADDR;

    for(uint8_t i = 0; i < MAX_SCORES; i++) {
        I2C_start(EEPROM_I2C_ADDR << 1);
        I2C_write(addr >> 8);    // High address byte
        I2C_write(addr & 0xFF);  // Low address byte
        I2C_write(i);            // Save index
        I2C_write(scoreHistory[i]); // Save score
        I2C_stop();
        Delay_Ms(5); // Wait for write completion

        addr += SCORE_SIZE; // Move to next slot
    }
}
...
void load_all_scores() {
    ...
    for(uint8_t i = 0; i < MAX_SCORES; i++) {
        ...
        uint8_t idx = I2C_read_ACK();  // Read index
        scoreHistory[idx] = I2C_read_NACK(); // Read score
        I2C_stop();

        addr += SCORE_SIZE; // Move to next slot
        Delay_Ms(1); // Short delay between reads
    }
}
